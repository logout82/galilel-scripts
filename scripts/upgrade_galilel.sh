...
wget -c https://github.com/Galilel-Project/galilel/releases/download/v$WALLET_VERSION/galilel-v$WALLET_VERSION-lin64.tar.gz >/dev/null 2>&1
  compile_error
echo -e "extract new wallet"  
  tar -xvzf galilel-v$WALLET_VERSION-lin64.tar.gz >/dev/null 2>&1

cd /root/galilel-v$WALLET_VERSION-lin64/usr/bin/ >/dev/null 2>&1
...
echo -e "copy new wallet to usr/local/bin"
  ...
  cd  >/dev/null 2>&1
 rm -r galilel-v$WALLET_VERSION-lin64* >/dev/null 2>&1
